...
public class RepositoryCreatorDialogView {
    ...
    public void initView(){
        ...
        //TreeTableView
        TreeItem<AbstractTreeItem> treeRootItem = new TreeItem<>(
                new RootTreeItem());
    ...
    private void buildTreeTableViewColumns() {
        ...
        treeTableView.getColumns().setAll(
                new IntervalColumn(intervalTreeItemFormatter),
                new DateColumn(),
                new IdColumn(),
                new TypeColumn(),
                new CategoryColumn(),
                new AmountColumn(),
                new DescriptionColumn(controller),
                new BankNameColumn(),
                new FileNameColumn());
    ...
    public void updateView( Operation operation, int index ) {
        ...
        Optional<TreeItem<AbstractTreeItem>> intervalTreeItemToAddOperation = findIntervalTreeItemOfOperation(currentInterval, intervals);
        TreeItem<AbstractTreeItem> newOperationTreeItem = new TreeItem<>(new OperationTreeItem(operation));
        if (intervalTreeItemToAddOperation.isPresent()){
            ...
            TreeItem<AbstractTreeItem> newIntervalTreeItem = new TreeItem<>(new IntervalTreeItem(operation.getRawOperation().getDate()));
    ...
    private Optional<TreeItem<AbstractTreeItem>> findIntervalTreeItemOfOperation( String currentInterval, ObservableList<TreeItem<AbstractTreeItem>> intervals ) {
